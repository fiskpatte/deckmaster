...
const DeckMap: React.FC<Props> = ({
  ...
  const [notReplacingCargoPlacements, setNotReplacingCargoPlacements] = useState<CargoPlacement[]>([]);
  const [replacingCargoPlacements, setReplacingCargoPlacements] = useState<CargoPlacement[]>([]);
  useEffect(() => {
  ...
  useEffect(() => {
    const notReplacingCargoPlacements = cargoPlacements.filter(cp => !cp.replacing) ?? [];
    const replacingCargoPlacements = cargoPlacements.filter(cp => cp.replacing) ?? []
    setNotReplacingCargoPlacements(notReplacingCargoPlacements)
    setReplacingCargoPlacements(replacingCargoPlacements)
  }, [cargoPlacements])

  useEffect(() => {
    setMostForwardValidPlacementForLanes(getMostForwardValidPlacementForLanes(deck.lanes, notReplacingCargoPlacements, currentCargoPlacement.cargo, bumperToBumperDistance, defaultVCG))
  }, [deck.lanes, notReplacingCargoPlacements, currentCargoPlacement, bumperToBumperDistance, defaultVCG])
